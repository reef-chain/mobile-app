...
const errorHandler = (message) => {
  ...
  if (message.includes("Module { index: 6, error: 2, message: None }") || message.includes('{"module":{"index":6,"error":2}}')) {
...
const captureError = (events) => {
  for (const event of events) {
    ...
    if (eventCompression === "evm.ExecutedFailed") {
      ...
      const message = hexToAscii(
        eventData[eventData.length - 2].substr(138)
      );
      return message;
    }
  }
