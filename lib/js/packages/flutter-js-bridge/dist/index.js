(() => {
  ...
  // src/FlutterJS.ts
  var FlutterJS = class {
    constructor(onInitFn) {
      if (onInitFn) {
        this.onInit = onInitFn;
      }
    }
    init(reefMobileChannelName, logStreamId, flutterSubscribeMethodName, apiReadyStreamId, apiTxSignStreamId, txSignatureConfirmationJsFnName, apiDappStreamId, dAppMsgConfirmationJsFnName) {
    ...
    sendToFlutterStream(streamId, value, msgType = "", reqId = "", jsChannelName) {
    ...
    sendFlutterSignatureRequest(reqId, msgType, value) {
    ...
    sendFlutterDAppMsgRequest(reqId, msgType, value) {
    ...
    registerMobileSubscriptionMethod(flutterSubscribeMethodName) {
      window[flutterSubscribeMethodName] = (observableRefName, subscriptionId) => {
        const isFn = observableRefName.indexOf("(");
        let observableRef;
        if (isFn > 0) {
          observableRef = window.eval(observableRefName);
        } else {
