    ...
    try {
      ...
      if (isMethod(method, 'approve') && isApproveContract(contractAddress)) {
        return loc.approveTransaction(
            amount, tokenSymbol, shortAddr(recipientAddress));
      } else if (isMethod(method, 'swap') && isSwapContract(contractAddress)) {
        return loc.swapTransaction(
            amount, tokenSymbol, shortAddr(recipientAddress));
        ...
        return loc.sendErc20Transaction(
            amount, tokenSymbol, shortAddr(recipientAddress));
      } else
      if (isMethod(method, 'sendNft') && isNftContract(contractAddress)) {
        final tokenId = request.payload
            .tokenId; // Assuming tokenId is in payload
        return loc.sendNftTransaction(tokenId, shortAddr(recipientAddress));
      } else if (isMethod(method, 'bindAccount')) {
        return loc.bindAccountTransaction(request.payload
            .accountDetails); // Assuming account details are in payload
      }
    }catch (e) {
      // TODO error here
