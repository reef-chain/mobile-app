...
class JsApiService {
  ...
  Future<WebViewController> getController() async {
    var res = await _getFlutterJsHeaderTags(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return WebViewFlutterJS(
            hidden: hiddenWidget,
            webviewFlutterWidget: WebViewWidget(controller: snapshot.data!),
          );
        } else {
          return Center(child: Text('No controller found.'));
        }
      },
    );
  }
  ...
  Future<void> jsCallVoidReturn(String executeJs) {
    return getController().then((ctrl) => ctrl.runJavaScript(executeJs));
  }

  Future<dynamic> jsCall<T>(String executeJs) async {
    try {
      dynamic res = await getController()
