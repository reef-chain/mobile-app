...
/// Defines the [ReefStepper]'s main axis.
enum StepperType {
...
///
/// Used by [ReefStepper.controlsBuilder].
@immutable
class ControlsDetails {
  ...
  /// Index that is active for the surrounding [ReefStepper] widget. This may be
  ...
  /// just activated the transition to a new step, some [ReefStepper.type] values will
...
/// A material step used in [ReefStepper]. The step can have a title and subtitle,
...
@immutable
class ReefStep {
  /// Creates a step for a [ReefStepper].
  ///
...
///
/// {@tool dartpad}
/// An example the shows how to use the [ReefStepper], and the [ReefStepper] UI
/// appearance.
///
...
class _ReefStepperState extends State<ReefStepper>
    with TickerProviderStateMixin {
  late List<GlobalKey> _keys;
  final Map<int, ReefStepState> _oldStates = <int, ReefStepState>{};
  ...
  @override
  void didUpdateWidget(ReefStepper oldWidget) {
  ...
  bool _isLabel() {
    for (final ReefStep step in widget.steps) {
      if (step.label != null) {
        return true;
      }
    }
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      return AnimatedCrossFade(
        ...
        crossFadeState: widget.steps[index].state == ReefStepState.error
            ? CrossFadeState.showSecond
            : CrossFadeState.showFirst,
        duration: kThemeAnimationDuration,
      );
    } else {
      if (widget.steps[index].state != ReefStepState.error) {
      ...
      children: <Widget>[
        for (int i = 0; i < widget.steps.length; i += 1)
          Column(
            ...
            children: <Widget>[
              InkWell(
                onTap: widget.steps[i].state != ReefStepState.disabled
                    ? () {
                    ...
                    : null,
                canRequestFocus:
                    widget.steps[i].state != ReefStepState.disabled,
      ...
      for (int i = 0; i < widget.steps.length; i += 1) ...<Widget>[
        InkResponse(
          onTap: widget.steps[i].state != ReefStepState.disabled
              ...
              : null,
          canRequestFocus: widget.steps[i].state != ReefStepState.disabled,
          child: Row(
            children: <Widget>[
              SizedBox(
  ...
  Widget build(BuildContext context) {
    ...
    assert(() {
      if (context.findAncestorWidgetOfExactType<ReefStepper>() != null) {
        throw FlutterError(
